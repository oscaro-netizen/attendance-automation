...
async def run_attendance_stop_automation(
    slack_user_id: str,
    ...
    async with AsyncSessionLocal() as db:
        service = AttendanceService(db)
        await service.process_stop_attendance(
            slack_user_id,
            slack_event_id,
            channel_id,
        )
...
def process_attendance_stop_task(
    self,
    slack_user_id: str,
    slack_event_id: str = None,
    channel_id: str = None,
    ...
    logger.info(
        f"Starting attendance stop task for user={slack_user_id}"
    )
    ...
    try:
        ...
        return loop.run_until_complete(
            run_attendance_stop_automation(
                slack_user_id,
                slack_event_id,
                channel_id,
            )
